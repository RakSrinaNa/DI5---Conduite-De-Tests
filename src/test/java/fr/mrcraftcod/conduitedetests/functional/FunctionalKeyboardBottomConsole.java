...
public class FunctionalKeyboardBottomConsole{
	...
	public void test() throws InvalidInput{
		...
		assertEquals("pushed 10, bottom stack is 10\n", bos.toString());
		bos.reset();
		
		input.actionCommandeInput("push 100");
		assertEquals("pushed 100, bottom stack is 10, 100\n", bos.toString());
		bos.reset();
		
		input.actionCommandeInput("push 1000");
		assertEquals("pushed 1000, bottom stack is 10, 100\n", bos.toString());
		bos.reset();
		
		input.actionCommandeInput("pop");
		assertEquals("popped 1000, bottom stack is 10, 100\n", bos.toString());
		bos.reset();
		
		input.actionCommandeInput("pop");
		assertEquals("popped 100, bottom stack is 10\n", bos.toString());
		bos.reset();
		
		input.actionCommandeInput("pop");
		assertEquals("popped 10, bottom stack is empty\n", bos.toString());
		bos.reset();
	}
}
